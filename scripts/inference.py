...
def inference(args, model, dev_dl, device, epoch, writer=None):
    ...
    import csv
    ipdb.set_trace()
    with open(os.path.join(args.save_dir, "id2label.json"), "w", encoding="utf8") as f:
    ...
    with open(pred_path, "w", encoding="utf8") as fout:
        for i, logit in enumerate(all_logits):
            k = all_charge_num[i] if all_charge_num is not None else 1
            ...
            if all_case_idx[i] != last_id:
                last_id = all_case_idx[i]
                id2accusations.append(txt)
                
            else:
                id2accusations[-1] += f";{txt}"
    ...
    with open(csv_path, "w", encoding="utf8", newline="") as csvfile:
        ...
        for idx in sorted(id2accusations.keys()):
            writer.writerow([idx, id2accusations[idx]])
